 ...
 */
 ...
 function Storestatus($status,$id){
    
$sql = $this->db->prepare("update application set status=:status where ID=:id");
 ...
 function Storeresponse($pname,$id){
    
$sql = $this->db->prepare("INSERT INTO application_response(`app_id`, `response`)
VALUES(:id,:pname)");
 ...
 */
      ...
      function Deleteclient($id)
    {
                ...
                $sql = $this->db->prepare("delete from register_users where ID=:id");
                $sql->bindParam(':id', $id);
                $sql->execute();
                if($sql)
                {
